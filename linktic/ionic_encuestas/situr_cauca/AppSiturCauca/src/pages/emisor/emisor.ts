...
export class EmisorPage {
  ...
  @ViewChild(EmisorMoment1Component) emisorMoment1Component:EmisorMoment1Component;
  ...
  save()
  {
    ...
    this.emisorMoment1Component.moment1.dateEnd=this.dateTime();
    this.emisor={"momento1":this.emisorMoment1Component.moment1,"lMomento2":lMomento2};
    if(lMomento2.length>0)
    {
      for(let moment2 of lMomento2)
      {
        if(moment2.lTrips.length>0)
        {
          for(let trip of moment2.lTrips)
          {
            if(trip.lDestination.length>0)
            {
              for(let destination of trip.lDestination)
              {
                destination.tempCity=null;
                destination.tempCountry=null;
                destination.tempState=null;
              }
            }
          }
        }
