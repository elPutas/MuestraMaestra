...
export class ReceptorPage {
  ...
  constructor(
    ...
    this.subscription=this._eventEmitter.eventReceptorSave.subscribe((flag:boolean)=>{
      if(flag)
      {
        this.save();
      }
  ...
  save()
  {
    ...
    this.receptor.momento2.dateEnd=this.dateTime();
    let flag=this.saveForm.save(this.receptor,this.appConfig.form1.number,this.appConfig.form1.name);
    if(flag)
    {
